...
- `index.html` references `/src/main.jsx`, which Vite resolves at build and dev
  time rather than serving as a static file. It is recorded as an expected
  baseline entry because it is correct, permanent, and not a broken link.
- `JS/VerAcc.js` and `JS/Signupresponder.js` remain on disk with their original
  broken import, but no page loads them any more: `VerAcc.html` and
  `Signupresponder.html` are now redirect stubs. The Firestore rules already
  reject the responder records those scripts tried to write. Deleting the files
  belongs to Phase 14.

## Harness limitation

`expectedMissingRefs` compares a `resolvedPath` that encodes how deep the
repository sits in the filesystem, so the baseline only matches on the machine
where it was captured. Running `npm run test:baseline` from a different path
reports differences in the `../` prefixes that are not real regressions.
